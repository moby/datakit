...
module PR = struct
  ...
  let of_gh repo pr =
    ...
    let user = User.v pr.pull_user.user_login in
    PR.v ~state ~title ~base ~user head pr.pull_number
  ...
  let of_event repo pr =
    ...
    let user = User.v pr.pull_request_event_pull_request.pull_user.user_login in
    PR.v ~state ~title ~base ~user head number
