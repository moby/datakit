...
let open_in ?cwd ?env ?stdin ?stderr cmd =
  match cwd with
  | None ->
    Lwt.return (Lwt_process.open_process_in ?env ?stdin ?stderr cmd)
  | Some dir ->
...
let run_with_exit_status ?switch ?log ?cwd ?env ?stdin ~output ?stderr ?log_cmd cmd =
...
let run ?switch ?log ?cwd ?env ?stdin ~output ?stderr ?log_cmd cmd =
  run_with_exit_status ?switch ?log ?cwd ?env ?stdin ~output ?stderr ?log_cmd cmd >|= fun status ->
  match switch with
