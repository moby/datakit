...
val config :
  ...
  can_build:CI_ACL.t ->
  unit -> t
(** [config ~name ~state_repo ()] is a web configuration. If [name] is
    given, it is used as the main heading, and also as the name of the
    session cookie (useful if you run multiple CIs on the same host,
    on different ports). If [state_repo] is given, it is used to
    construct links to the state repository on GitHub. *)

val pr_url : CI_projectID.t -> int -> string
(** [pr_url project pr] is a link to the page for [pr]. *)

val ref_url : CI_projectID.t -> string -> string
