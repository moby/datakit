## com.docker.db: a Git-like database with a 9p interface

This directory contains two binaries:

- an Git-like database with a 9p interface: `com.docker.db`; and
- a small script to mount 9p endpoint to access the databaase

### Build

At the root of this repository, write `make opam`. This will produces
`db.native` and `mount.native` in the current directory.
...
```
./db.native --git=/tmp/db.git  # starts the DB using /tmp/db.git for storage
