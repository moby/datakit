...
module type CLIENT = sig
  ...
  module Transaction : sig
    (** All changes to a branch are made in transactions. When a transaction is committed, it is merged with
    ...
    val create_file : t -> dir:Datakit_path.t -> string -> ?executable:bool -> Cstruct.t -> unit or_error Lwt.t
    (** [create_file t ~dir name ?executable content] creates the file [dir/name]. *)

    val create_symlink : t -> dir:Datakit_path.t -> string -> string -> unit or_error Lwt.t
    (** [create_symlink t ~dir name target] creates the symlink [dir/name]. *)

    val replace_file : t -> dir:Datakit_path.t -> string -> Cstruct.t -> unit or_error Lwt.t
    (** [replace_file t ~dir name new_content] changes the content of the existing file [dir/name]. *)
    ...
    (** [rename t path new_name] changes the basename of [path] to [new_name].
    ...
    (** [truncate t path length] sets the length of the file at [path] to [length].
        If [length] is longer than the current length, the file is padded with zero bytes. *)
    ...
    (** [commit t ~message] creates a new commit with the given log message and the current contents of the transaction and then
        merges it into the branch from which the transaction was created.
        The transaction cannot be used after calling this. *)

    val abort : t -> unit Lwt.t
    (** [abort t] aborts the transaction without committing it.
        The transaction cannot be used after calling this. *)
    ...
    (** When performing a merge, these three directories can be used to calculate the final result.
        [ours] is the previous contents of the transaction, [theirs] is the commit being merged and [base]
        is a least common ancestor. If there is no common ancestor then [base] is an empty tree. *)

    val merge : t -> Commit.t -> (merge_inputs * Datakit_path.t list) or_error Lwt.t
    (** [merge t commit] merges [commit] into the transaction. It performs any trivial merges it can and returns
        [(merge_inputs, conflicts)] to allow you to resolve the remaining ones. You must write to each path in [conflicts]
        at least once before you can commit the transaction.
        You may perform multiple merges in one transaction, but the [merge_inputs] returned is only valid until the start
        of the next merge. *)
    ...
    (** [set_parents t new_parents] replaces the current list of parents.
        Note that this does not perform a merge - it only affects the metadata.
        Note also that [merge] automatically updates the parents, so it is not necessary to call it manually in that case. *)

    val conflicts : t -> Datakit_path.t list or_error Lwt.t
    (** [conflicts t] returns the current list of paths that had merge conflicts and have not been written to since.
        It is not possible to commit while this is non-empty. *)
  end
  ...
  module Branch : sig
    ...
    val wait_for_head : t -> ?switch:Lwt_switch.t -> (Commit.t option -> [`Finish of 'a | `Again | `Abort] or_error Lwt.t) ->
      [`Abort | `Finish of 'a] or_error Lwt.t
    (** [wait_for_head t fn] calls [fn commit] on the current commit. If it returns [`Again] then
        it waits for the commit to change and tries again.
        If [switch] is provided, then turning off the switch will make the wait return [`Abort] at the next opportunity. *)
    ...
    (** [with_transaction t fn] is the result of applying [fn] to a new transaction on branch [t].
        If the transaction has not been committed when [fn trans] returns, the transaction is aborted
  ...
  (** [remove_branch t name] removes the branch named [name] (unlike [Branch.remove], this method doesn't
      require creating the branch directory first). *)
