...
module Git_fs_store = struct
  ...
  let connect path =
    ...
    repo path >|= fun repo () -> Filesystem.create ~info repo
end
...
module In_memory_store = struct
  ...
  let connect () =
    ...
    repo () >|= fun repo () -> Filesystem.create ~info repo
end
...
let start ~listen_9p prometheus git =
  ...
  let serve_9p =
    ...
    List.map
      ...
      listen_9p
  in
  serve_9p >>= fun serve_9p -> Lwt.choose (serve_9p @ prometheus_threads)
    ...
    | Some remote -> (
      ...
      | Some local ->
          ...
          Git_fs_store.Store.Branch.watch_all repo (fun branch _ ->
              ...
              Lwt.return_unit )
          >>= fun w -> start () >>= fun () -> Git_fs_store.Store.unwatch w ) )
