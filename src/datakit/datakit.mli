...
module Dir (Store: S): sig
  ...
  val update:
    t -> Path.t -> string -> Blob.t * [Metadata.t | `Keep] ->
  ...
  val rename: t -> path -> old_name:string -> new_name:string ->
    (unit, [`Is_a_directory | `Not_a_directory | `No_such_item]) result Lwt.t
