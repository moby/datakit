...
module Sync (API: API) (DK: Datakit_S.CLIENT): sig
  ...
  val sync: API.token -> DK.t -> branch:string -> unit Lwt.t
  (** [sync token dk br] monitors changes in the DataKit instance
      [dk], using the branch [br]. It ensures a bi-directional
      consistency between GitHub API and DataKit. It connects to the
      GitHub API using the token [tok ()]. *)

end

module API: API with type token = Github.Token.t
(** [API] implementation using [ocaml-github]. *)
